...
export default function Faq() {
    ...
    const renderListCatelogy = (val, index) => {
        const btnType = val === selectedCategory && searchValue === '' ? 'primary' : 'default'
        return (
            <Button key={index} className='btnCatelogy' type={btnType} shape="round" onClick={()=>onClickCategory(val)}>{val}</Button> 
        )
    }
    ...
    const renderCollapse = () => {
        ...
        return listCatelogy.map((val, index) => {
            if(val === selectedCategory) {
                return (
                    <Collapse key={index} expandIconPosition={'right'} defaultActiveKey={[1]}>
                        {renderPanel()}
                    </Collapse>
                )
            }
            ...
            <Row>
                <Col xs={{span: 24}}  sm={{span: 24}} lg={{span: 16, offset: 4}}>
                    <div className='rowCatelogy'>
                        {listCatelogy.map((val, index)=>renderListCatelogy(val, index))}
                    </div>
                    {renderCollapse()}
                 </Col>
            </Row>
